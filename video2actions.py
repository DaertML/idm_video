...
def predict_video_actions(video_path, model):
    """
    Processes a video file frame by frame, predicts the action between F_t and F_t+1,
    displays the annotated video, and saves frames grouped by predicted action sequences.
    """
    ...
    # Tracking variables for sequence saving
    last_predicted_action_label = None
    sequence_counter = 0
    sequence_save_dir = None
    
    # Read the first frame (F_t)
    ...
    F_t_preprocessed = preprocess_frame(frame_t_rgb)
    ...
    while True:
        ...
        # 4. Sequence Tracking and Folder Creation (Requirement 2)
        
        # Check if the predicted action has changed
        if predicted_action_label != last_predicted_action_label:
            sequence_counter += 1
            # Define the new folder path for this sequence
            sequence_save_dir = os.path.join(
                OUTPUT_DIR, 
                predicted_action_label, 
                f"sequence_{sequence_counter:04d}" # Sequence name with leading zeros
            )
            # Create the directory structure (e.g., predicted_actions_output/FIRE/sequence_0001)
            os.makedirs(sequence_save_dir, exist_ok=True)
            last_predicted_action_label = predicted_action_label
            
        # 5. Display the results and save the annotated frame (Requirement 1 & Display)
        
        # We display and save the F_t_plus_1 frame
        display_frame = frame_t_plus_1_rgb 
        ...
        # Save the frame to the current sequence folder
        # Use the global frame_count in the filename for unique identification
        filename = f"frame_{frame_count:05d}.png"
        cv2.imwrite(os.path.join(sequence_save_dir, filename), display_frame)

        # Display the frame in the viewer
        cv2.imshow('IDM Action Prediction', display_frame)
    ...
    print(f"\nVideo processing finished after {frame_count} frames.")
    print(f"Total sequences detected: {sequence_counter}")


if __name__ == '__main__':
    print("--- Inverse Dynamics Model Prediction and Sequence Extraction ---")
