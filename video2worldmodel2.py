...
def predict_video_actions(video_path, model):
    ...
    as frame0.png (F_t) and frame1.png (F_t+1, unannotated).
    """
    ...
    while True:
        ...
        # 5. Display the results and save the pair
        
        # We prepare a copy of F_t+1 for display and annotation ONLY
        display_frame = frame_t_plus_1_rgb.copy() 
        
        # Overlay the predicted action onto the display copy
        ...
        # Save F_t+1 (the frame *after* the action started/completed) (UNANNOTATED) as frame1.png
        # We use the raw frame_t_plus_1_rgb here, which has no text overlay.
        ft_plus_1_filename = "frame1.png"
        cv2.imwrite(os.path.join(sequence_save_dir, ft_plus_1_filename), frame_t_plus_1_rgb)

        # Display the F_t+1 frame (ANNOTATED) in the viewer
